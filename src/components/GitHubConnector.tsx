...
export function GitHubConnector({ appId, folderName }: GitHubConnectorProps) {
  ...
  // Add state for public/private toggle
      ...
      <div className="mt-4 w-full border border-gray-200 rounded-md p-4">
        ...
        <form className="mt-4 space-y-2" onSubmit={handleCreateRepo}>
          ...
          <div className="flex items-center gap-2">
            <span className={`text-sm font-medium ${!isPrivate ? 'text-blue-600' : 'text-gray-500'}`}>Public</span>
            <Switch
              checked={isPrivate}
              onCheckedChange={setIsPrivate}
              id="repo-privacy-switch"
                      />
            <span className={`text-sm font-medium ${isPrivate ? 'text-blue-600' : 'text-gray-500'}`}>Private</span>
            </div>
            <Button
              type="submit"
